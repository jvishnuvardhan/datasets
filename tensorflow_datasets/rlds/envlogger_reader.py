...
def get_episode_dict(episode: Sequence[Any],
                     ...
                     episode_fn: Callable[[Dict[str, Any]],
                                          Dict[str, Any]] = lambda x: x,
                     steps_to_ds=False) -> Optional[Dict[str, Any]]:
  ...
  return episode_fn(episode_dict)
...
def generate_episodes(
  ...
  for index, episode_metadata in enumerate(tag_reader.episode_metadata()):
    ...
    episode_dict = get_episode_dict(episode, episode_metadata, keep_episode_fn,
                                    step_fn, episode_fn, steps_to_ds)
    if episode_dict:
      yield episode_dict
...
def _build_empty_step(
  ...
  rlds_step = tf.nest.map_structure(np.zeros_like, get_step_metadata(step))
