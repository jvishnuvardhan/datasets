 ...
 * `normal` ("Istella LETOR"): Containing 10,454,629 query-document pairs.
 * `s` ("Istella-S LETOR"): Containing 3,408,630 query-document pairs.
 * `x` ("Istella-X LETOR"): Containing 26,791,447 query-document pairs.

You can specify whether to use the `normal`, `s` or `x` version of the dataset
as follows:

```python
ds = tfds.load("istella/normal")
ds = tfds.load("istella/s")
ds = tfds.load("istella/x")
```

If only `istella` is specified, the `istella/normal` option is selected by
default:
```python
# This is the same as `tfds.load("istella/normal", split="train")`
...
_URLS = {
    "normal": "http://library.istella.it/dataset/istella-letor.tar.gz",
...
class Istella(tfds.core.GeneratorBasedBuilder):
  ...
  BUILDER_CONFIGS = [
      IstellaConfig(name="normal", has_vali=False, subdirectory="full"),
